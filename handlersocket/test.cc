...
hstcpcli_main(int argc, char **argv)
{
  ...
  const std::string dbname = conf.get_str("dbname", "mapkeeper");
  const std::string table = conf.get_str("table", "usertable");
  const std::string index = conf.get_str("index", "PRIMARY");
  const std::string fields = conf.get_str("fields", "record_key,record_value");
  const int limit = conf.get_int("limit", 0);
  const int skip = conf.get_int("skip", 0);
  std::vector<std::string> keys;
  std::vector<string_ref> keyrefs;
  size_t num_keys = 1;
  std::string key = "user102642hello";
  keys.push_back(key);
